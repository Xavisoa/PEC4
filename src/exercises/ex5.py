...
def add_points(data):
    ''' Añade las columnas points_home y points_away al DataFrame con los puntos obtenidos en cada partido '''
...
def fun_total_points(data):
    ...
    # Ordenamos los equipos de mayor a menor número de puntos
    total_points = total_points.sort_values(ascending=False)

    # Creamos un DataFrame utilizando los nombres de los equipos en el índice y los puntos como valores
...
def alltime_winner(df_total_points):
    ''' Devuelve el equipo con más puntos históricos '''

    # Como df_total_points está ordenado del equipo con más puntos al equipo con menos
